...
def reload_olts(kwargs):
    ...
    try:
        ...
        for i in range(len(os.listdir(hda_folder))):
            filename = os.listdir(hda_folder)[i]
            files = os.listdir(hda_folder).sort()
            if filename.lower().endswith(".hdanc"):
                if files[i].split(".")[0] == files[i+1].split(".")[0] and files[i].split(".")[1] == files[i+1].split(".")[1] and files[i].split(".")[2] == files[i+1].split(".")[2]:
                    passing = True
                    ivtop = os.listdir(hda_folder)[i].split(".")[3]
                    ivbot = os.listdir(hda_folder)[i].split(".")[4]
                    tvtop = os.listdir(hda_folder)[i+1].split(".")[3]
                    tvbot = os.listdir(hda_folder)[i+1].split(".")[4]
                    if ivtop <= tvtop and ivbot <=tvbot:
                        passing = False
                    if passing == True:
